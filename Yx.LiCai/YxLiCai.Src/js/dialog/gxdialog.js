/*!
 * @description:gxDialog ---> Global-x dialog
 * @author:Eilvein
 * @version:V1.0.0
 * @update:2015/5/26
 */
...
define(['jquery'],function(require, exports, module) {
	...
	var $ = require("jquery");//jquery库

	/*!
	 *
	 * 简易调用：$.gxDialog(string, [ok, no])
	 * 完整方法：$.gxDialog(opt)
	...
	(function(factory){
		...
		dialog.init = function(opt, ok, no){
			...
			_this.dom.box = $('<div></div>', {'id': 'gxdialog', 'class': 'gxdialog'});
			_this.dom.overlay = $('<div></div>', {'id': 'gxdialog_overlay', 'class': 'gxdialog_overlay'});
			_this.dom.holder = $('<div></div>', {'id': 'gxdialog_holder', 'class': 'gxdialog_holder'});
			_this.dom.title = $('<div></div>', {'class': 'gxdialog_title'});
			_this.dom.info = $('<div></div>', {'class': 'gxdialog_info'});
			_this.dom.btns = $('<div></div>', {'class': 'gxdialog_btns'});
			_this.dom.closeBtn = $('<a></a>', {'rel': 'gxdialog', 'rev': 'close'});
		...
		dialog.format = function(opt, ok, no){
			...
			opt = $.extend({}, $.gxDialog.defaults, opt);
		...
		dialog.setContent = function(opt){
	      ...
	      _this.dom.info.html(opt.info).appendTo(_this.dom.box);
			...
			if (_this.btnCache.length) {
				var _html = '';
				for (var i = 0, l = _this.btnCache.length; i < l; i++) {
					_html += '<a class="' + _this.btnCache[i].className + '" rel="gxdialog" rev="' + _this.btnCache[i].name + '">' + _this.btnCache[i].text + '</a>';
				};
		...
		dialog.show = function(opt){
			...
			if (opt.lockScroll === true) {
				_this.dom.docHtml.addClass('gxdialog_lock');
			};
		...
		dialog.exit = function(){
			...
			_this.dom.box.attr('class', 'gxdialog').empty().hide();
			_this.dom.overlay.hide();
			_this.dom.docHtml.removeClass('gxdialog_lock');
		};
