...
define('uc_setting_status', ['jquery', 'gxdialog','spin'], function (require, exports, module) {
    ...
    function autoCloseDialog() {
        $.gxDialog({
            ...
            width: 280,
            closeBtn: false,
            info: '<div class="pop-box"><h3>认证成功</h3><p>一休哥提示，您实名认证成功<p><div class="cd-time">3</div></div>'
        });
        var secondNum = 3;
        loadJump(secondNum);
    ...
    $("#ok-bank").click(function () {
        ...
        if ($("#Name").val().replace(/[ ]/g, "") == "") { alertDialog("请输入姓名"); return; } else { $("#message").empty(); }
        if (manCode == "") { alertDialog("请输入身份证号码"); return; } else { $("#message").empty(); }
        ...
        if (!checkIdcard(manCode)) { alertDialog("身份证号码输入错误，请重新输入"); return; } else { $("#message").empty(); }
        var opts = {
        };
