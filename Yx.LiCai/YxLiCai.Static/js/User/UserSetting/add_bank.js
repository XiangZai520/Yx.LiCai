...
define('add_bank', ['jquery', 'gxdialog', 'jcookie'], function (require, exports, module) {
    ...
    function sendMessage() {
        ...
        var card = $("#MyRealCard").val().replace(/[ ]/g, "");
        var realname = $("#MyRealName").val().replace(/[ ]/g, "");
        if (BankName == "") { alertDialog("请选择开户银行"); return; }
        if (BankCard == "") { alertDialog("请输入银行卡号"); return; }
        if (!luhmCheck(BankCard)) { alertDialog("银行卡输入不正确，请修改"); return; }
        if (Phone == "") { alertDialog("请输入手机号"); return; }
        if (!isMobel(Phone)) { alertDialog("手机号输入不正确，请修改"); return; }
        $(".yx-tips").css("display", "");
        $.ajax({
            type: "POST",
            async: false,
            url: "/UserSetting/SendBoundBankCard",
            data: { BankName: BankName, BankCard: BankCard, Phone: Phone, Card: card, RealName: realname },
            success: function (data) {
                if (data.Status == 1) {
                    ...
                    var lastphone = Phone.substr(parseInt(length - 4));
                    $(".yx-tips").html("我们已将验证码发送至您****" + lastphone + "的手机，请耐心等待。");
                } else {
    ...
    $("#ok-bank").click(function () {
        ...
        var reg = /^(\d{15,19})$/g;
        if (BankCard == "") { alertDialog("请输入银行卡号"); return; }
        if (!reg.test(BankCard)) { alertDialog("银行卡输入不正确，请修改"); return; }
        if (Phone == "") { alertDialog("请输入手机号"); return; }
        if (!isMobel(Phone)) { alertDialog("手机号输入不正确，请修改"); return; }
        var phoneCode = $("#phoneCode").val().replace(/[ ]/g, "");
        if (phoneCode == "") { alertDialog("请输入手机验证码"); return; }
        if (!checkIdcard(card)) { alertDialog("身份证格式不正确"); return; }
        ...
        $.ajax({
            ...
            data: { BankName: BankName, BankCard: BankCard, Phone: Phone, validatecode: phoneCode, Card: card, RealName: realname },
            success: function (data) {
                if (data.Status == 1) {
                    if (returnUrl != "") {
