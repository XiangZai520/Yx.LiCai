...
namespace YxLiCai.Tools.Pay.Yeepay
{
    ...
    public class YJPayUtil
    {
            ...
            string encryptkey, bool post)
        {
            ...
            string yibaoPublickey = YeepayConfig.getYeepayPublicKey();
        ...
        public static string checkYbResult(String ybResult)
        {
            ...
            {
                //将支付结果json字符串反序列化为对象
                RespondJson respJson = SerializeHelper.JsonDeserialize<RespondJson>(ybResult);
                ...
                if (passSign)
                {
                    ...
                    string payresult_view = AES.AesDecrypt(yb_data, yb_aeskey);
        ...
        /// <param name="ybResult">易宝支付返回的结果</param>
        /// <returns></returns>
        public static string checkYbCallBackResult(string yb_data, string yb_encryptkey)
        { 
            //将易宝返回的结果进行验签名
            bool passSign = EncryptUtil.checkDecryptAndSign(yb_data, yb_encryptkey, yibaoPublickey, merchantPrivatekey);
            if (passSign)
