...
namespace YxLiCai.Tools
{
    public static class XmlHelper
    {
        ...
        /// <summary>
        /// 从XML字符串中反序列化对象
        /// Override [XmlRoot(Namespace = "", ElementName = "")]
        /// </summary>
        /// <typeparam name="T">结果对象类型</typeparam>
        /// <param name="s">包含对象的XML字符串</param>
        /// <param name="elementName">XmlRootAttribute.ElementName </param>
        /// <param name="encoding">编码方式</param>
        /// <returns>反序列化得到的对象</returns>
        public static T XmlDeserialize<T>(string s, string elementName, Encoding encoding)
        {
            if (string.IsNullOrEmpty(s))
                throw new ArgumentNullException("s");
            if (encoding == null)
            ...
            using (var ms = new MemoryStream(encoding.GetBytes(s)))
            {
                using (var sr = new StreamReader(ms, encoding))
                {
                    ...
                    var mySerializer = new XmlSerializer(typeof(T), myXmlAttributeOverrides);
